...
/* Shared test cases for subprocess.h.

   Each test translation unit must define SUBPROCESS_SUITE before including this
   header: #define SUBPROCESS_SUITE c #include "test_shared.h"

   The SUBPROCESS_TEST macro expands SUBPROCESS_SUITE before UTEST token
   pasting, giving each standard-specific translation unit unique test symbols.
*/

#ifndef SUBPROCESS_SUITE
#error "Define SUBPROCESS_SUITE before including test_shared.h"
#endif

#define SUBPROCESS_TEST_NAME3(set, name) set##_##name
#define SUBPROCESS_TEST_NAME2(set, name) SUBPROCESS_TEST_NAME3(set, name)
#define SUBPROCESS_TEST2(suite, name) UTEST(suite, name)
#define SUBPROCESS_TEST(set, name) \
  SUBPROCESS_TEST2(SUBPROCESS_SUITE, SUBPROCESS_TEST_NAME2(set, name))
...
SUBPROCESS_TEST(create, subprocess_destroy_is_idempotent) {
...
SUBPROCESS_TEST(create_ex, subprocess_create_failure_preserves_error) {
#if defined(_WIN32)
...
SUBPROCESS_TEST(create, subprocess_return_zero) {
  const char *const commandLine[] = {"./process_return_zero", 0};
  struct subprocess_s process;
  int ret = -1;
...
SUBPROCESS_TEST(create, subprocess_return_fortytwo) {
...
SUBPROCESS_TEST(create, subprocess_return_argc) {
  const char *const commandLine[] = {
...
SUBPROCESS_TEST(create, subprocess_return_argv) {
...
SUBPROCESS_TEST(create, subprocess_return_stdin) {
  ...
  ASSERT_EQ(UTEST_CAST(int, 'a'), putc('a', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'b'), putc('b', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'b'), putc('b', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'a'), putc('a', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, ' '), putc(' ', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'a'), putc('a', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'r'), putc('r', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'e'), putc('e', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, ' '), putc(' ', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'g'), putc('g', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'r'), putc('r', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'e'), putc('e', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 'a'), putc('a', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, 't'), putc('t', stdin_file));
  ASSERT_EQ(UTEST_CAST(int, '!'), putc('!', stdin_file));
...
SUBPROCESS_TEST(create, subprocess_return_stdin_count) {
...
SUBPROCESS_TEST(create, subprocess_stdout_argc) {
  const char *const commandLine[] = {
...
SUBPROCESS_TEST(create, subprocess_stdout_argc_with_empty_strings) {
  const char *const commandLine[] = {
...
SUBPROCESS_TEST(create, subprocess_stdout_argv) {
  const char *const commandLine[] = {
...
SUBPROCESS_TEST(create, subprocess_stderr_argc) {
  const char *const commandLine[] = {
...
SUBPROCESS_TEST(create, subprocess_stderr_argv) {
  const char *const commandLine[] = {
...
SUBPROCESS_TEST(create, subprocess_return_lpcmdline) {
...
SUBPROCESS_TEST(create, subprocess_combined_stdout_stderr) {
...
SUBPROCESS_TEST(create, subprocess_not_inherit_environment) {
...
#else
  static char environment[] = "PROCESS_ENV_TEST=1";
  ASSERT_FALSE(putenv(environment));
#endif
...
SUBPROCESS_TEST(create, subprocess_inherit_environment) {
  const char *const commandLine[] = {"./process_inherit_environment", 0};
  struct subprocess_s process;
  int ret = -1;
...
#else
  static char environment[] = "PROCESS_ENV_TEST=42";
  ASSERT_FALSE(putenv(environment));
#endif
...
SUBPROCESS_TEST(create, subprocess_not_inherit_all_environment) {
...
SUBPROCESS_TEST(create, subprocess_inherit_all_environment) {
  const char *const commandLine[] = {"./process_inherit_environment", "all", 0};
  struct subprocess_s process;
  int ret = -1;
...
SUBPROCESS_TEST(create, subprocess_fail_divzero) {
...
SUBPROCESS_TEST(create, subprocess_fail_stackoverflow) {
...
SUBPROCESS_TEST(create, subprocess_hung) {
...
SUBPROCESS_TEST(create, subprocess_read) {
  ...
  for (index = 0; index < sizeof(data) - 1; index++) {
    ...
    ASSERT_TRUE(table[index % 16] == data[index]);
  }
...
SUBPROCESS_TEST(subprocess, stdout_fgetc) {
...
SUBPROCESS_TEST(subprocess, stdout_fread_all) {
...
SUBPROCESS_TEST(subprocess, read_stdout) {
  const char *const commandLine[] = {"./process_stdout_large", "16384", 0};
  struct subprocess_s process;
  int ret = -1;
  static char data[1048576 + 1] = {0};
...
SUBPROCESS_TEST(subprocess, read_stdout_async_small) {
...
SUBPROCESS_TEST(subprocess, read_stdout_async) {
  const char *const commandLine[] = {"./process_stdout_large", "16384", 0};
  struct subprocess_s process;
  int ret = -1;
  static char data[1048576 + 1] = {0};
...
SUBPROCESS_TEST(subprocess, read_stdout_async_no_wait_requires_async) {
  const char *const commandLine[] = {"./process_return_zero", 0};
  struct subprocess_s process;

  ASSERT_NE(0, subprocess_create(commandLine,
                                 subprocess_option_enable_async_no_wait,
                                 &process));
}

SUBPROCESS_TEST(subprocess, poll_stdout_async) {
  ...
  while (alive || (0 != bytes_read)) {
    bytes_read = subprocess_read_stdout(&process, data + index,
                                        sizeof(data) - 1 - index);
    index += bytes_read;

    // Send the control character once to tell the subprocess to stop after
    // we've read at least one thing from its stdout.
    if ((0 != bytes_read) && !sent_stop) {
      ASSERT_NE(EOF, fputc('s', subprocess_stdin(&process)));
      ASSERT_EQ(0, fflush(subprocess_stdin(&process)));
      sent_stop = 1;
    }
...
SUBPROCESS_TEST(subprocess, poll_stdout_async_wait_first) {
  ...
  // We first have zero bytes read because we haven't written the control
  // character to stdin.
...
SUBPROCESS_TEST(subprocess, poll_stderr_async) {
  ...
  while (alive || (0 != bytes_read)) {
    ...
    // Send the control character once to tell the subprocess to stop after
    // we've read at least one thing from its stderr.
    if ((0 != bytes_read) && !sent_stop) {
      ASSERT_NE(EOF, fputc('s', subprocess_stdin(&process)));
      ASSERT_EQ(0, fflush(subprocess_stdin(&process)));
      sent_stop = 1;
...
SUBPROCESS_TEST(subprocess, poll_stderr_async_wait_first) {
...
SUBPROCESS_TEST(create, subprocess_alive) {
  const char *const commandLine[] = {"./process_return_stdin_count", 0};
  struct subprocess_s process;
  int ret = -1;
  FILE *stdin_file;
...
SUBPROCESS_TEST(create, subprocess_alive_then_join) {
  const char *const commandLine[] = {"./process_return_zero", 0};
  struct subprocess_s process;
  int ret = -1;
...
SUBPROCESS_TEST(environment, illegal_inherit_environment) {
  const char *const commandLine[] = {"./process_return_zero", 0};
...
SUBPROCESS_TEST(environment, illegal_empty_environment_with_inherit_environment) {
  const char *const commandLine[] = {"./process_return_zero", 0};
...
SUBPROCESS_TEST(environment, null_environment_with_inherit_environment) {
  const char *const commandLine[] = {"./process_return_zero", 0};
  struct subprocess_s process;
...
SUBPROCESS_TEST(environment, specify_environment) {
  const char *const commandLine[] = {"./process_inherit_environment", 0};
...
SUBPROCESS_TEST(create_ex, subprocess_cwd) {
  ...
  ASSERT_TRUE(subprocess_test_getcwd(current_path, sizeof(current_path)) != NULL);
...
SUBPROCESS_TEST(executable_resolve, no_slashes_with_inherit) {
  const char *const commandLine[] = {"process_inherit_environment", 0};
  struct subprocess_s process;
  int ret = -1;
...
#if !defined(_MSC_VER)
SUBPROCESS_TEST(executable_resolve, custom_search_path) {
  char current_path[4096];
  ...
  ASSERT_TRUE(getcwd(current_path, sizeof(current_path)) != NULL);
...
#if !defined(_MSC_VER) && !defined(__MINGW32__)
SUBPROCESS_TEST(executable_resolve, missing_from_path) {
  const char *const commandLine[] = {"process_call_return_argc", 0};
  struct subprocess_s process;
  int ret = -1;
...
#if !defined(_MSC_VER)
SUBPROCESS_TEST(executable_resolve, default_search_path) {
...
SUBPROCESS_TEST(create, subprocess_option_no_window) {
  const char *const commandLine[] = {"./process_return_zero", 0};
  struct subprocess_s process;
  int ret = -1;
